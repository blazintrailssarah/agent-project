...
| `scripts/ci-local.sh`                                         | Modified    | Fixed local review reliability issues (commit messages file generation + ANSI-safe summary output)                           |
| `.crewai/main.py`                                             | Modified    | Fixed fallback CI status interpretation (`passed` derived from `status` when absent)                                         |
...
| Local CI review   | ✅ Verified | End-to-end `./scripts/ci-local.sh --review` completes; fallback behavior handles external model credit failures |
...
- [x] ~~Run and capture successful `./scripts/ci-local.sh --review` output after latest fixes~~ — Done: pipeline passes end-to-end with resilient fallback when model credits are unavailable
...
_Last updated: 2026-02-13 19:02_
